...
def train(args, model, device, train_loader, optimizer, epoch, num_samples):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = model(data.unsqueeze(1).type(torch.FloatTensor))
...
def main():
    ...
    val_sampler = WeightedRandomSampler(torch.DoubleTensor(climb_dataset.get_weights()), len(climb_dataset), replacement=False)
