...
class CNNLSTM_Net(nn.Module):
    ...
    def forward(self, frames):
        ...
        hidden = (torch.zeros(1, f, 128).to(frames.device), torch.zeros(1, f, 128).to(frames.device))
        out = []
...
class CNNBiLSTM_Net(nn.Module):

    def __init__(self):

        super(CNNBiLSTM_Net, self).__init__()
        ...
        # LSTM
        self.rnn = nn.LSTM(input_size=128, hidden_size=128, num_layers=1, bidirectional=True)
    ...
    def forward(self, frames):
        ...
        # initialize hidden state and final output
        hidden = (torch.zeros(2, f, 128).to(frames.device), torch.zeros(2, f, 128).to(frames.device))
        out = []
        ...
        rnn_output, hidden = self.rnn(cnn_cat, hidden)
        #print("RNN_output: ", rnn_output.size())
        rnn_output = rnn_output.reshape(f, 1, 256)
...
class CNNBiLSTM_pf_Net(nn.Module):

    def __init__(self):

        super(CNNBiLSTM_pf_Net, self).__init__()
    ...
    def forward(self, frames):
        ...
        # send final RNN output to compute whether send / fail
        # print("Last RNN output: ", rnn_output[-1].size())
